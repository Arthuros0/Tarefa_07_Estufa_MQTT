...
#define WIFI_SSID "SuaRedeWiFi"                  // Substitua pelo nome da sua rede Wi-Fi
#define WIFI_PASSWORD "SuaSenha"      // Substitua pela senha da sua rede Wi-Fi
#define MQTT_SERVER "SEU_IP"                // Substitua pelo endereço do host - broket MQTT: Ex: 192.168.1.107
#define MQTT_USERNAME "SeuUsuario"     // Substitua pelo nome da host MQTT - Username
#define MQTT_PASSWORD "SuaSenha"     // Substitua pelo Password da host MQTT - credencial de acesso - caso exista
...
static void estufas_worker_fn(async_context_t *context, async_at_time_worker_t *worker);
static async_at_time_worker_t estufas_worker = { .do_work = estufas_worker_fn };
...
int main(void) {
    ...
    button_init_predefined(true,true,true); //Inicializa os botões A, B e SW (Joystick)

    joystick_setup();

    button_setup_irq(true,true,true,&button_callback); //Configura interrupção para os botões A, B e SW (Joystick)
...
static void publish_estufas(MQTT_CLIENT_DATA_T *state) {
    if (update_data) {
        char payload[256];
        char topic[MQTT_TOPIC_LEN];
        ...
        for (uint8_t i = 0; i < NUM_ESTUFAS; i++) {
            // --- Dados ambientais ---
            snprintf(topic, sizeof(topic), "estufa/%d/dados", estufas[i].id);
            snprintf(payload, sizeof(payload),
                     "{\"temperatura\":%.1f,\"umidade_ar\":%.1f,\"umidade_solo\":%.1f}",
                     estufas[i].temperatura,
                     estufas[i].umidade_ar,
                     estufas[i].umidade_solo);
            mqtt_publish(state->mqtt_client_inst, full_topic(state, topic), payload, strlen(payload),
                         MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);

            // --- Status dos atuadores ---
            snprintf(topic, sizeof(topic), "estufa/%d/status", estufas[i].id);
            snprintf(payload, sizeof(payload),
                     "{\"aspersor\":\"%s\",\"irrigacao\":\"%s\",\"ventilacao\":\"%s\",\"fertirrigacao\":\"%s\"}",
                     estufas[i].status_umidade ? "ligado" : "desligado",
                     estufas[i].status_irrigacao ? "ligado" : "desligado",
                     estufas[i].status_ventilacao ? "ligado" : "desligado",
                     estufas[i].status_fertilizante ? "ligado" : "desligado");
            mqtt_publish(state->mqtt_client_inst, full_topic(state, topic), payload, strlen(payload),
                         MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
        }
    }
...
static void sub_unsub_topics(MQTT_CLIENT_DATA_T* state, bool sub) {
    ...
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "estufa/1/fertirrigacao"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "estufa/2/fertirrigacao"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "estufa/3/fertirrigacao"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
}
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags) {
    ...
    } else if (strncmp(basic_topic, "estufa/", 7) == 0 && strstr(basic_topic, "/fertirrigacao") != NULL) {
        // Extração do ID da estufa a partir do tópico
        uint8_t id_estufa = 0;
        if (sscanf(basic_topic, "estufa/%hhu/fertirrigacao", &id_estufa) == 1) {
            // IDs de estufas são de 1 a NUM_ESTUFAS, índice no vetor vai de 0 a NUM_ESTUFAS-1
            if (id_estufa >= 1 && id_estufa <= NUM_ESTUFAS) {
                alterna_fertirrigacao(id_estufa - 1); // vetor começa do 0
                INFO_printf("Fertirrigação alternada para estufa %d\n", id_estufa);
            } else {
                INFO_printf("ID de estufa fora do intervalo: %d\n", id_estufa);
            }
        } else {
            INFO_printf("Falha ao extrair ID da estufa do tópico: %s\n", basic_topic);
        }
    }
}
...
static void mqtt_connection_cb(mqtt_client_t *client, void *arg, mqtt_connection_status_t status) {
    ...
    if (status == MQTT_CONNECT_ACCEPTED) {
        ...
        estufas_worker.user_data = state;
        async_context_add_at_time_worker_in_ms(cyw43_arch_async_context(), &estufas_worker, 0);
